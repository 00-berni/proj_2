...
plt.figure()
x_ticks = np.arange(len(rec_lum))*5
...
plt.grid(linestyle='dashed',color='gray',alpha=0.5)
plt.xticks(x_ticks,np.round(rec_lum*1e4,0))
plt.xlabel('$\\ell_{rec}$ [$10^{-4}$]',fontsize=FONTSIZE)
...
s0_cnts = len(check[check ==   0])
s1_cnts = len(check[check <=   ker_sigma])
s2_cnts = len(check[check <= 2*ker_sigma])
s3_cnts = len(check[check <= 3*ker_sigma])

print('COUNTS')
print('s0',s0_cnts)
print('s1',s1_cnts,s1_cnts-s0_cnts)
print('s2',s2_cnts,s2_cnts-s1_cnts)
print('s3',s3_cnts,s3_cnts-s2_cnts)
